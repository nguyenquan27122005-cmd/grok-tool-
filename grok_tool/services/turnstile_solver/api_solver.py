...
class TurnstileAPIServer:
    ...
    def __init__(self, headless: bool, useragent: Optional[str], debug: bool, browser_type: str, thread: int, proxy_support: bool, use_random_config: bool = False, browser_name: Optional[str] = None, browser_version: Optional[str] = None):
        ...
        # force_terminal=False + emoji=False avoids Windows GBK crashes when
        # stdout is redirected (service manager / logs) and welcome text
        # contains non-GBK glyphs from upstream branding.
    ...
    def display_welcome(self):
        ...
        ASCII-only on purpose: Windows service launches often inherit a GBK
        console code page, and emoji in the upstream banner abort startup.
        """
        try:
            combined_text = Text()
            ...
            # Never let banner rendering kill the HTTP service.
            logger.warning("Welcome banner skipped: %s", type(exc).__name__)
    ...
    async def _solve_turnstile(
        ...
        try:
            ...
            # Prefer per-task proxy (registration egress); fall back to proxies.txt pool.
            task_proxy = (proxy or '').strip() or None
            if task_proxy:
                proxy = task_proxy
            else:
