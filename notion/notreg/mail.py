"""Acquire inbox + wait Notion login code / magic link. tmail.wibucrypto.pro only."""

from __future__ import annotations

import json
import re
import time
from typing import Any
from urllib.parse import unquote

import requests

from notreg.log import log
from notreg.paths import ROOT, ensure_grok_on_path
from notreg.stop import raise_if_stop

BLOCKED_FILE = ROOT / "data" / "blocked_tmail_domains.json"

HINTS = (
    "notion",
    "notify@mail.notion",
    "mail.notion.so",
    "login code",
    "sign in to notion",
    "log in to notion",
    "verification",
    "verify",
    "magic",
    "temporary password",
    "one-time",
    "otp",
)
CODE_RE = re.compile(r"\b(\d{6})\b")
CODE_LABELED = re.compile(
    r"(?:login code|verification code|code is|otp|mã(?:\s+xác\s+nhận)?)\s*(?:is|:)?\s*(\d{4,8})",
    re.I,
)
LINK_RE = re.compile(
    r"https?://(?:www\.)?notion\.so/[^\s\"'<>]*(?:verify|login|signup|magic|token|confirm)[^\s\"'<>]*",
    re.I,
)
ANY_LINK = re.compile(r"https?://(?:www\.)?notion\.so/[^\s\"'<>]{12,}", re.I)
_SKIP = {"000000", "123456", "111111", "999999"}
GUERRILLA_API = "https://api.guerrillamail.com/ajax.php"


def _blob_is_target(text: str) -> bool:
    low = (text or "").lower()
    return any(h in low for h in HINTS)


def extract_proof(text: str) -> dict[str, str]:
    raw = unquote(text or "")
    out: dict[str, str] = {}
    m = LINK_RE.search(raw) or ANY_LINK.search(raw)
    if m:
        link = m.group(0).rstrip(").,]>\"'")
        if not any(x in link.lower() for x in (".css", ".js", ".png", ".svg", "/help", "help.notion")):
            out["link"] = link
    m = CODE_LABELED.search(raw)
    if m and m.group(1) not in _SKIP:
        out["code"] = m.group(1)
        return out
    if "code" not in out and "notion" not in raw.lower():
        return out
    for c in CODE_RE.findall(raw):
        if c in _SKIP:
            continue
        if len(c) == 4 and c.startswith("20"):
            continue
        out["code"] = c
        break
    return out


class GuerrillaMail:
    def __init__(self) -> None:
        self._http = requests.Session()

    def create(self):
        from grokreg.mail.mail_api import EmailSession

        r = self._http.get(GUERRILLA_API, params={"f": "get_email_address"}, timeout=20)
        r.raise_for_status()
        d = r.json() or {}
        email = str(d.get("email_addr") or "").strip()
        sid = str(d.get("sid_token") or "").strip()
        if not email or not sid:
            raise RuntimeError(f"Guerrilla create fail: {d}")
        log.info("Guerrilla ready: %s", email)
        return EmailSession(
            address=email,
            password="",
            provider="guerrilla",
            token=sid,
            extra={"sid_token": sid, "email_addr": email},
        )

    def list_messages(self, session) -> list[dict[str, Any]]:
        sid = (session.extra or {}).get("sid_token") or session.token
        r = self._http.get(
            GUERRILLA_API,
            params={"f": "check_email", "seq": "0", "sid_token": sid},
            timeout=20,
        )
        r.raise_for_status()
        return list((r.json() or {}).get("list") or [])

    def fetch_body(self, session, mail_id: str) -> str:
        sid = (session.extra or {}).get("sid_token") or session.token
        r = self._http.get(
            GUERRILLA_API,
            params={"f": "fetch_email", "email_id": str(mail_id), "sid_token": sid},
            timeout=20,
        )
        r.raise_for_status()
        return str((r.json() or {}).get("mail_body") or "")


def load_blocked_domains(config: dict[str, Any] | None = None) -> list[str]:
    blocked: set[str] = set()
    if BLOCKED_FILE.exists():
        try:
            raw = json.loads(BLOCKED_FILE.read_text(encoding="utf-8"))
            if isinstance(raw, list):
                blocked.update(str(x).strip().lower() for x in raw if str(x).strip())
            elif isinstance(raw, dict):
                blocked.update(
                    str(x).strip().lower()
                    for x in (raw.get("domains") or [])
                    if str(x).strip()
                )
        except Exception as e:
            log.debug("blocked domains read: %s", e)
    tw = (config or {}).get("tmail_wibu") or {}
    for x in tw.get("skip_domains") or []:
        if str(x).strip():
            blocked.add(str(x).strip().lower())
    return sorted(blocked)


def save_blocked_domains(domains: list[str]) -> None:
    BLOCKED_FILE.parent.mkdir(parents=True, exist_ok=True)
    BLOCKED_FILE.write_text(
        json.dumps(sorted({str(d).strip().lower() for d in domains if str(d).strip()}), indent=2),
        encoding="utf-8",
    )


def ban_domain(config: dict[str, Any], email_or_domain: str) -> str:
    domain = (email_or_domain or "").split("@")[-1].strip().lower()
    if not domain:
        return ""
    blocked = set(load_blocked_domains(config))
    blocked.add(domain)
    save_blocked_domains(list(blocked))
    tw = config.setdefault("tmail_wibu", {})
    tw["skip_domains"] = sorted(blocked)
    pref = [d for d in (tw.get("domains") or []) if str(d).strip().lower() not in blocked]
    tw["domains"] = pref
    log.warning("Ban tmail domain %s — remaining pref=%s blocked=%s", domain, pref, len(blocked))
    return domain


def acquire_email(config: dict[str, Any]):
    grok = ensure_grok_on_path()
    if not grok:
        raise RuntimeError("Thiếu folder grok_tool cạnh notion — cần tmail.wibucrypto.pro / Hotmail")

    want = str(config.get("email_provider") or "tmail_wibu").strip().lower()
    if want in ("5", "custom", "custom_domain", "domain", "rieng"):
        want = "custom_domain"

    if want == "custom_domain":
        # Domain riêng: forward→Hotmail → đọc OTP qua Graph
        from grokreg.core.config import load_config as load_grok_cfg
        from grokreg.mail.mail_api import MailApiClient
        from grokreg.mail.providers import AzpopMailProvider, MailTmProvider
        from grokreg.mail.tmail_wibu import TmailWibuProvider
        from grokreg.reg.flow import acquire_email_session

        grok_cfg: dict[str, Any] = {}
        try:
            grok_cfg = load_grok_cfg()
        except Exception:
            grok_cfg = {}
        merged = dict(grok_cfg)
        merged.update(config)
        merged["email_provider"] = "custom_domain"
        hl = str(config.get("hotmail_list") or "data/hotmails.txt")
        hp = ROOT / hl
        if not hp.exists() and (grok / "data" / "hotmails.txt").exists():
            merged["hotmail_list"] = str(grok / "data" / "hotmails.txt")
        else:
            merged["hotmail_list"] = hl
        mailtm = MailTmProvider()
        azpop = AzpopMailProvider(dict(grok_cfg.get("azpopmail") or {}))
        tmail = TmailWibuProvider(dict(grok_cfg.get("tmail_wibu") or {}))
        session, hotmail = acquire_email_session(merged, mailtm, azpop, tmail_wibu=tmail)
        log.info("Notion custom_domain: %s (OTP đọc qua Hotmail %s)", session.address, getattr(session, "mailbox", "?"))
        return session, hotmail, MailApiClient(merged.get("mail_api") or {}), azpop, tmail, mailtm, None

    # Notion: tmail.wibucrypto.pro only — never Hotmail / Azpop / Guerrilla
    config["email_provider"] = "tmail_wibu"
    tmail_cfg = dict(config.get("tmail_wibu") or {})
    tmail_cfg["base_url"] = str(tmail_cfg.get("base_url") or "https://tmail.wibucrypto.pro")
    tmail_cfg["create_mode"] = str(tmail_cfg.get("create_mode") or "create")
    blocked = load_blocked_domains(config)
    tmail_cfg["skip_domains"] = blocked
    sfx = list(tmail_cfg.get("skip_suffixes") or [])
    tmail_cfg["skip_suffixes"] = [str(s).strip().lower().lstrip(".") for s in sfx if str(s).strip()]
    pref = [d for d in (tmail_cfg.get("domains") or []) if str(d).strip().lower() not in set(blocked)]
    tmail_cfg["domains"] = pref
    config["tmail_wibu"] = tmail_cfg
    tmail = TmailWibuProvider(tmail_cfg)
    address, extra = tmail.create_mailbox()
    session = EmailSession(
        address=address,
        password="",
        provider="tmail_wibu",
        token="",
        extra=extra,
    )
    log.info("Notion mail tmail: %s", session.address)
    return session, None, MailApiClient({}), None, tmail, None, None


def _graph_messages(session, client_id: str) -> list[dict[str, Any]]:
    rt = getattr(session, "refresh_token", "") or ""
    cid = getattr(session, "client_id", "") or client_id
    if not rt:
        return []
    tok = requests.post(
        "https://login.microsoftonline.com/common/oauth2/v2.0/token",
        data={
            "client_id": cid,
            "grant_type": "refresh_token",
            "refresh_token": rt,
            "scope": "https://graph.microsoft.com/Mail.Read offline_access",
        },
        timeout=25,
    )
    if tok.status_code != 200:
        log.warning("Graph token HTTP %s", tok.status_code)
        return []
    access = (tok.json() or {}).get("access_token") or ""
    if not access:
        return []
    inbox = requests.get(
        "https://graph.microsoft.com/v1.0/me/mailFolders/inbox/messages"
        "?$top=20&$orderby=receivedDateTime desc"
        "&$select=id,subject,from,body,bodyPreview,receivedDateTime$select=id,subject,from,toRecipients,ccRecipients,body,bodyPreview,receivedDateTime",
        headers={"Authorization": f"Bearer {access}"},
        timeout=25,
    )
    if inbox.status_code != 200:
        return []
    return list((inbox.json() or {}).get("value") or [])


def _msg_text(m: dict[str, Any]) -> str:
    body = m.get("body")
    html = ""
    if isinstance(body, dict):
        html = str(body.get("content") or "")
    return " ".join(
        [
            str(m.get("subject") or ""),
            str(m.get("bodyPreview") or ""),
            html,
            str((m.get("from") or {}).get("emailAddress", {}).get("address") or ""),
        ]
    )


def _msg_to(m: dict[str, Any], addr: str) -> bool:
    """Mail gửi tới đúng địa chỉ (custom domain forward — 1 hộp đọc chung)."""
    want = (addr or "").strip().lower()
    if not want:
        return True
    for key in ("toRecipients", "ccRecipients"):
        for item in m.get(key) or []:
            if isinstance(item, dict):
                a = str((item.get("emailAddress") or {}).get("address") or "").lower()
                if a == want:
                    return True
    blob = f"{str(m.get('subject') or '')} {str(m.get('bodyPreview') or '')}".lower()
    return want in blob


def wait_mail(
    session,
    config: dict[str, Any],
    *,
    mail_api=None,
    hotmail=None,
    azpop=None,
    tmail=None,
    mailtm=None,
    guerrilla=None,
    timeout: int = 180,
) -> dict[str, str]:
    deadline = time.time() + max(8, int(timeout))
    provider = str(getattr(session, "provider", "") or "")
    cid = str((config.get("mail_api") or {}).get("client_id") or "")
    if int(timeout) >= 20:
        log.info("Chờ mail Notion (%s, timeout=%ss)…", provider, timeout)

    while time.time() < deadline:
        raise_if_stop()
        try:
            if provider == "guerrilla" and guerrilla:
                for m in guerrilla.list_messages(session) or []:
                    subj = str(m.get("mail_subject") or "")
                    frm = str(m.get("mail_from") or "")
                    body = ""
                    mid = str(m.get("mail_id") or "")
                    if mid:
                        try:
                            body = guerrilla.fetch_body(session, mid)
                        except Exception:
                            pass
                    blob = f"{subj} {frm} {body}"
                    if not _blob_is_target(blob):
                        continue
                    proof = extract_proof(blob)
                    if proof:
                        log.info("Notion mail Guerrilla: %s", proof)
                        return proof
            elif provider in ("hotmail", "custom_domain"):
                target = (
                    str(getattr(session, "address", "") or "").lower()
                    if provider == "custom_domain"
                    else ""
                )
                for m in _graph_messages(session, cid):
                    blob = _msg_text(m)
                    if not _blob_is_target(blob):
                        continue
                    if target and not _msg_to(m, target):
                        continue
                    proof = extract_proof(blob)
                    if proof:
                        log.info("Notion mail %s: %s", provider, proof)
                        return proof
            elif provider == "azpopmail" and azpop:
                extra = session.extra or {}
                user = extra.get("username") or session.address.split("@")[0]
                domain = extra.get("domain") or session.address.split("@")[-1]
                msgs, _ = azpop._list_messages(user, domain)
                for m in msgs or []:
                    mid = str(m.get("id") or m.get("mail_id") or "")
                    body = ""
                    try:
                        body = azpop._message_body(user, domain, mid)
                    except Exception:
                        pass
                    blob = f"{m.get('subject','')} {m.get('from','')} {body}"
                    if not _blob_is_target(blob) and not extract_proof(blob):
                        continue
                    proof = extract_proof(blob)
                    if proof:
                        log.info("Notion mail Azpop: %s", proof)
                        return proof
            elif provider == "tmail_wibu" and tmail:
                extra = dict(getattr(session, "extra", None) or {})
                page_html = ""
                try:
                    _, page_html, page_email = tmail._scrape_mailbox_page(extra)
                    if page_email and str(page_email).lower() != str(session.address).lower():
                        log.warning("tmail page email %s != %s", page_email, session.address)
                except Exception as e:
                    log.warning("tmail scrape: %s", e)
                proof = extract_proof(page_html or "")
                if proof:
                    log.info("Notion mail tmail page: %s", proof)
                    return proof
                try:
                    messages, html_blob = tmail._fetch_messages(session.address, extra)
                    blob = html_blob or ""
                    if messages:
                        log.info("tmail inbox %s msg(s)", len(messages))
                    left = int(deadline - time.time())
                    if int(deadline - time.time()) % 21 < 4:
                        log.info(
                            "tmail empty=%s left=%ss html=%s",
                            0 if messages else 1,
                            left,
                            "Empty Inbox" in (page_html or html_blob or ""),
                        )
                    for msg in messages or []:
                        blob += " " + tmail._msg_blob(msg)
                        mid = str(msg.get("id") or msg.get("uid") or "")
                        if mid and hasattr(tmail, "_open_message"):
                            try:
                                blob += " " + (tmail._open_message(session.address, extra, mid) or "")
                            except Exception:
                                pass
                    proof = extract_proof(blob)
                    if proof:
                        log.info("Notion mail tmail: %s", proof)
                        return proof
                except Exception as e:
                    log.warning("tmail fetch: %s", e)
                    try:
                        tmail._rehandshake_into(session.address, extra)
                    except Exception as e2:
                        log.warning("tmail re-handshake: %s", e2)
            elif mailtm:
                otp = mailtm.wait_otp(session, timeout=8)
                if otp:
                    return {"code": str(otp)}
        except Exception as e:
            log.debug("mail poll: %s", e)
        time.sleep(3)
    return {}
