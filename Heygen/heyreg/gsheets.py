"""Push HeyGen success ledger → Google Sheet tab `heygen` (same layout as grok)."""
...
from heyreg.paths import DATA, GROK_ROOT, ROOT
...
def append_heygen_account(
    email: str,
    password: str,
    status: str,
    ts: str = "",
) -> str:
    """Một acc thành công → 1 dòng trên tab heygen (không ghi đè cả bảng)."""
    gs = load_gs_config()
...
def export_heygen_to_sheet() -> str:
    gs = load_gs_config()
    if not gs.get("enabled", True):
        return "google_sheets disabled"
