"""CLI: python main.py 0 --count 1 --backend protocol"""
...
from heyreg.config import load_config
from heyreg.log import log, setup_logging
from heyreg.stop import clear_stop, request_stop
from heyreg.worker import run_batch


def _pick_mail(cli: str | None, cfg: dict) -> str:
    if cli in ("1", "hotmail"):
        return "hotmail"
    # tmail: bat lai voi domain rotation + ban domain khong nhan mail HeyGen
...
def main(argv: list[str] | None = None) -> int:
    setup_logging()
    p = argparse.ArgumentParser(description="HeyGen register")
    p.add_argument("mail", nargs="?", default="0", help="0=temp 1=hotmail 2=azpop 3=tmail 5=domain riêng")
    ...
    p.add_argument("--backend", "-b", choices=("browser", "protocol", "auto"), default="protocol")
    ...
    clear_stop()
    log.info(
        "HeyGen reg mail=%s backend=%s count=%s",
        cfg["email_provider"],
        cfg.get("reg_backend"),
