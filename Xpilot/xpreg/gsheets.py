"""Push X-Pilot success ledger → Google Sheet tab `xpilot` (same layout as grok)."""
...
from xpreg.paths import DATA, GROK_ROOT, ROOT
...
def append_xpilot_account(
    email: str,
    password: str,
    status: str,
    ts: str = "",
) -> str:
    """Một acc thành công → 1 dòng trên tab xpilot (không ghi đè cả bảng)."""
    gs = load_gs_config()
...
def export_xpilot_to_sheet() -> str:
    gs = load_gs_config()
    if not gs.get("enabled", True):
        return "google_sheets disabled"
