...
from claudereg.config import resolve_password
from claudereg.log import log
from claudereg.mail import acquire_email, wait_mail
from claudereg.paths import ROOT
from claudereg.stop import StopRequested, raise_if_stop
...
def _run_backend(config: dict[str, Any], email: str, password: str, wait_fn) -> dict[str, Any]:
    ...
    if backend in ("protocol", "http"):
        ...
        from claudereg.protocol import register_protocol

        return register_protocol(config, email=email, password=password, wait_mail=wait_fn)
    if backend == "gpm":
        import asyncio

        from claudereg.browser import register_gpm
    ...
    from claudereg.browser import register_browser
...
def register_one(config: dict[str, Any]) -> Result:
    ...
    try:
        ...
        if ok:
            try:
                from claudereg.gsheets import append_claude_account

                msg = append_claude_account(
                    email,
                    password,
                    status,
                    ts,
                ...
                log.info("Google Sheet claude: %s", str(msg)[:180])
    ...
    def _worker(wid: int) -> list[Result]:
        ...
        while True:
            ...
            try:
                ...
                return local
            log.info("======== CLAUDE [L%s] lượt %s/%s ========", wid + 1, cur, total)
...
def run_batch(config: dict[str, Any], count: int, *, resume: bool = False, threads: int = 1) -> list[Result]:
    ...
    if resume and checkpoint:
        st = _state_load()
        if st.get("tool") == "claude" and int(st.get("pending") or 0) > 0:
    ...
    for i in range(1, n + 1):
        raise_if_stop()
        log.info("======== CLAUDE %s / %s ========", i, "∞" if until_stop else n)
        r = register_one(config)
        if not r.ok and _transient(r) and int(config.get("retry_transient") or 1):
            log.warning("Lỗi tạm thời (%s) — thử lại ngay 1 lần", r.status)
            r = register_one(config)
