...
from claudereg.paths import ROOT, ensure_grok_on_path
...
LINK_RE = re.compile(
    r"https?://(?:www\.)?(?:claude\.ai|console\.anthropic\.com)/[^\s\"'<>]+",
    re.I,
)
...
def acquire_email(config: dict[str, Any]):
    ...
    if not grok:
        raise RuntimeError("Thiếu folder grok_tool cạnh claude — cần mail")
...
def wait_mail(
    ...
    *,
    ...
    log.info("Chờ mail Claude OTP (%s, timeout=%ss)…", provider, timeout)
