...
from claudereg.stop import clear_stop, request_stop
...
def main(argv: list[str] | None = None) -> int:
    setup_logging()
    p = argparse.ArgumentParser(description="Claude / Anthropic register (claude.ai)")
    ...
    clear_stop()
    log.info(
        "Claude reg mail=%s backend=%s count=%s",
        cfg["email_provider"],
        cfg.get("reg_backend"),
