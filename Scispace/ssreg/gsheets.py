"""Push SciSpace success ledger → Google Sheet tab `scispace` (same layout as grok)."""
...
from ssreg.paths import DATA, GROK_ROOT, ROOT
...
def append_scispace_account(
    email: str,
    password: str,
    status: str,
    ts: str = "",
) -> str:
    """Một acc thành công → 1 dòng trên tab scispace (không ghi đè cả bảng)."""
    gs = load_gs_config()
...
def export_scispace_to_sheet() -> str:
    gs = load_gs_config()
    if not gs.get("enabled", True):
        return "google_sheets disabled"
...
if __name__ == "__main__":
    print(export_scispace_to_sheet())
