...
from manreg.config import resolve_password
from manreg.log import log
from manreg.mail import acquire_email, wait_mail
from manreg.paths import ROOT
from manreg.stop import StopRequested, raise_if_stop, request_stop
...
def _run_backend(config: dict[str, Any], email: str, password: str, wait_fn) -> dict[str, Any]:
    ...
    if backend in ("protocol", "http"):
        ...
        from manreg.protocol import register_protocol
    ...
    from manreg.browser import register_browser

    use_gpm = backend == "gpm"
    log.info("Backend %s", "GPM (anti-detect)" if use_gpm else "BROWSER")
    return asyncio.run(register_browser(config, email=email, password=password, wait_mail=wait_fn, use_gpm=use_gpm))
...
def register_one(config: dict[str, Any]) -> Result:
    ...
    try:
        ...
        cred = sess.get("credits") if isinstance(sess.get("credits"), dict) else {}
        if cred.get("summary"):
            offer = str(cred.get("summary"))
            detail = (offer + " " + detail).strip()[:200]
        _save(save, email, password, status, detail)
        if ok:
            try:
                from manreg.gsheets import append_account
                ...
                msg = append_account(
                    ...
                    extra=offer or detail,
                )
                log.info("Google Sheet manus: %s", str(msg)[:180])
    ...
    def _worker(wid: int) -> list[Result]:
        ...
        while True:
            ...
            try:
                ...
                return local
            log.info("======== MANUS [L%s] lượt %s/%s ========", wid + 1, cur, total)
...
def run_batch(config: dict[str, Any], count: int, *, resume: bool = False, threads: int = 1) -> list[Result]:
    ...
    if resume and checkpoint:
        st = _state_load()
        if st.get("tool") == "manus" and int(st.get("pending") or 0) > 0:
    ...
    for i in range(1, n + 1):
        raise_if_stop()
        log.info("======== MANUS %s / %s ========", i, "∞" if until_stop or until_success else n)
        r = register_one(config)
        if not r.ok and _transient(r) and int(config.get("retry_transient") or 1):
            log.warning("Lỗi tạm thời (%s) — thử lại ngay 1 lần", r.status)
            r = register_one(config)
