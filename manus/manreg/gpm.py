...
def create_profile(config: dict[str, Any], name: str, proxy: str = "") -> str:
    """Tạo profile GPM mới — trả id. Route đúng của bản GPM này là
    GET /v2/create?name=… (toàn bộ v2 dùng GET: create/start/stop/delete);
    POST /api/v1/profiles/create trong tài liệu cũ KHÔNG tồn tại — nó rớt
    vào trang catch-all trả text "GPM-Login" (đã gặp thật, mất một buổi dò)."""
    base = api_base(config)
    params: dict[str, str] = {"name": name}
    if proxy:
        # GPM nhận http/socks5 scheme; bare host:port nó coi là http.
        params["raw_proxy"] = proxy

    def _do(p: dict[str, str]) -> dict[str, Any]:
        r = requests.get(f"{base}/v2/create", params=p, timeout=30)
        r.raise_for_status()
        text = (r.text or "").strip()
        if not text.startswith("{"):
            raise RuntimeError(f"route /v2/create không có (trả {text[:40]!r})")
        return json.loads(text)

    try:
        payload = _do(params)
        if isinstance(payload, dict) and payload.get("status") is False:
            raise RuntimeError(str(payload.get("message") or payload)[:160])
    except Exception as exc:
        if not proxy:
            raise RuntimeError(f"GPM tạo profile {name!r} fail: {exc}") from exc
        # proxy bậy khiến create chối → thử lại không proxy cho thông
        try:
            payload = _do({"name": name})
        except Exception as e2:
            raise RuntimeError(f"GPM tạo profile {name!r} fail: {e2}") from e2
    pid = str(payload.get("profile_id") or "") if isinstance(payload, dict) else ""
    if not pid:
        data = payload.get("data") if isinstance(payload, dict) else None
        pid = str((data or {}).get("id") or "")
    if not pid:
        raise RuntimeError(f"GPM tạo profile không trả id: {str(payload)[:200]}")
    log.info("[gpm] đã tạo profile %s (%s)", name, pid)
    return pid


def ensure_profile(config: dict[str, Any], name: str) -> str:
    """Có profile theo tên thì dùng, chưa có thì tạo mới qua GET /v2/create
    (kèm raw_proxy nếu config có). Tạo hỏng vì lý do gì thì rớ về profile có
    sẵn — reg vẫn chạy, chỉ là dùng chung fingerprint với profile đó."""
    try:
        return resolve_profile_id({**config, "gpm_profile": name})
    except RuntimeError:
        pass
    try:
        return create_profile(config, name, proxy=str(config.get("proxy") or ""))
    except Exception as e:
        log.warning("[gpm] tạo profile %s không được (%s) — dùng profile có sẵn.", name, e)
        return resolve_profile_id(config)
