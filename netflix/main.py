...
from nfreg.config import load_config
from nfreg.log import log, setup_logging
from nfreg.stop import clear_stop, request_stop
from nfreg.worker import run_batch
...
def main(argv: list[str] | None = None) -> int:
    setup_logging()
    p = argparse.ArgumentParser(description="Netflix register (stops at payment wall)")
    p.add_argument("mail", nargs="?", default="1", help="1=hotmail 2=azpop 3=tmail 4=guerrilla 5=domain riêng")
    ...
    p.add_argument("--until-success", action="store_true", help="Chạy đến khi tới payment wall")
    ...
    clear_stop()
    log.info(
        "Netflix reg mail=%s backend=%s count=%s until_success=%s (payment out of scope)",
        cfg["email_provider"],
        cfg.get("reg_backend"),
