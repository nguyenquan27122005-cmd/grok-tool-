"""Acquire inbox + wait Netflix verification (code or link)."""
...
from nfreg.paths import ROOT, ensure_grok_on_path
...
CODE_LABELED = re.compile(
    r"(?:verification code|sign-in code|code is|otp|mã(?:\s+xác\s+nhận)?)\s*(?:is|:)?\s*(\d{4,8})",
    re.I,
)
# Official signup/login links + household confirm URLs (help.netflix.com + IMAP tools on GitHub).
LINK_RE = re.compile(
    r"https?://(?:www\.)?netflix\.com/[^\s\"'<>]*"
    r"(?:verify|confirm|activate|token|login|signup|simpleSetup|URL_SIGNUP|lnktrk)[^\s\"'<>]*",
    re.I,
)
ANY_NF_LINK = re.compile(r"https?://(?:www\.)?netflix\.com/[^\s\"'<>]{12,}", re.I)
...
def extract_proof(text: str) -> dict[str, str]:
    raw = unquote(text or "")
    out: dict[str, str] = {}
    m = LINK_RE.search(raw) or ANY_NF_LINK.search(raw)
    if m:
        link = m.group(0).rstrip(").,]>\"'")
        if not any(x in link.lower() for x in (".css", ".js", ".png", ".svg", "/asset/", "help.netflix")):
            out["link"] = link
...
def acquire_email(config: dict[str, Any]):
    ...
    if not grok:
        raise RuntimeError("Thiếu folder grok_tool cạnh netflix — cần mail Azpop/Hotmail")
...
def wait_mail(
    ...
    *,
    ...
    log.info("Chờ mail Netflix (%s, timeout=%ss)…", provider, timeout)
        ...
        try:
            if provider == "guerrilla" and guerrilla:
                for m in guerrilla.list_messages(session) or []:
                    ...
                    if not _blob_is_target(blob) and "netflix" not in blob.lower():
                        continue
                    proof = extract_proof(blob)
                    if proof:
                        log.info("Netflix mail Guerrilla: %s", proof)
                        return proof
                ...
                for m in _graph_messages(session, cid):
                    ...
                    proof = extract_proof(blob)
                    if proof:
                        log.info("Netflix mail %s: %s", provider, proof)
                        return proof
                ...
                for m in msgs or []:
                    ...
                    proof = extract_proof(blob)
                    if proof:
                        log.info("Netflix mail Azpop: %s", proof)
                        return proof
            elif provider == "tmail_wibu" and tmail:
                extra = dict(getattr(session, "extra", None) or {})
                try:
                    _, page_html, _ = tmail._scrape_mailbox_page(extra)
                    proof = extract_proof(page_html or "")
                    if proof:
                        log.info("Netflix mail tmail page: %s", proof)
                        return proof
                ...
                try:
                    ...
                    proof = extract_proof(blob)
                    if proof:
                        log.info("Netflix mail tmail: %s", proof)
                        return proof
                except Exception as e:
