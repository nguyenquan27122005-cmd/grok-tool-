...
from gsparkreg.config import resolve_password
from gsparkreg.log import log
from gsparkreg.mail import acquire_email, wait_mail
from gsparkreg.paths import ROOT
from gsparkreg.stop import StopRequested, raise_if_stop
...
def _run_backend(config: dict[str, Any], email: str, password: str, wait_fn) -> dict[str, Any]:
    ...
    if backend in ("protocol", "http"):
        ...
        from gsparkreg.protocol import register_protocol

        return register_protocol(config, email=email, password=password, wait_mail=wait_fn)
    if backend == "gpm":
        import asyncio

        from gsparkreg.browser import register_gpm

        log.info("Backend GPM (D:\\gpm)")
        return asyncio.run(register_gpm(config, email=email, password=password, wait_mail=wait_fn))
    if backend == "auto":
        from gsparkreg.protocol import register_protocol
    ...
    from gsparkreg.browser import register_browser
...
def register_one(config: dict[str, Any]) -> Result:
    ...
    try:
        ...
        def _wait(timeout: int | None = None, after_ts: float | None = None) -> dict[str, str]:
            return wait_mail(
                session,
                config,
                mail_api=mail_api,
                ...
                mailtm=mailtm,
                guerrilla=guerrilla,
                timeout=int(timeout or config.get("timeout_otp") or 180),
                after_ts=after_ts,
            )
        ...
        detail = str(out.get("detail") or "")[:240]
        if out.get("session"):
            try:
        ...
        extra = detail if ok else ""
        _save(save, email, password, status, extra)

        if ok:
            try:
                from gsparkreg.gsheets import append_genspark_account

                msg = append_genspark_account(
                    email,
                    password,
                    status,
                    ts,
                ...
                log.info("Google Sheet genspark: %s", str(msg)[:180])
...
def _backend_parallel_ok(config: dict[str, Any]) -> bool:
    ...
    return backend in ("protocol", "http", "pure_http", "")
    ...
    def _worker(wid: int) -> list[Result]:
        ...
        while True:
            ...
            try:
                ...
                return local
            log.info("======== GENSPARK [L%s] lượt %s/%s ========", wid + 1, cur, total)
...
def run_batch(config: dict[str, Any], count: int, *, resume: bool = False, threads: int = 1) -> list[Result]:
    ...
    if resume and checkpoint:
        st = _state_load()
        if st.get("tool") == "genspark" and int(st.get("pending") or 0) > 0:
    ...
    for i in range(1, n + 1):
        raise_if_stop()
        log.info("======== GENSPARK %s / %s ========", i, "∞" if until_stop else n)
        r = register_one(config)
        if not r.ok and _transient(r) and int(config.get("retry_transient") or 1):
            log.warning("Lỗi tạm thời (%s) — thử lại ngay 1 lần", r.status)
            r = register_one(config)
